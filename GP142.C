...
static void GP_clear(void)
{
    ...
    HBRUSH hBrush, hOldBrush;
    hBrush = CreateSolidBrush(gRGBColors[BLACK]);
    hOldBrush = SelectObject(offscreen_DC, hBrush);
...
int count = 0;
void GP142_bucket_fill(int x, int y, int old_c, int new_c)
{
	//count++;
	//x = gXCenter + x;
	//y = gYCenter - y;
	...
	if ( new_c != old_c)
	{
		if (get_p_color(x, y) == old_c)
		{
			GP142_pixelXY(new_c, x, y);
			GP142_bucket_fill(x + 1, y, old_c, new_c);
			GP142_bucket_fill(x - 1, y, old_c, new_c);
			GP142_bucket_fill(x, y + 1, old_c, new_c);
			GP142_bucket_fill(x, y - 1, old_c, new_c);
			//GP142_bucket_fill( x -2,  y - 2, old_c, new_c);
			//GP142_bucket_fill( x-2,  y + 2, old_c, new_c);
			//GP142_bucket_fill( x+2,  y - 2, old_c, new_c);
			//GP142_bucket_fill( x+2,  y + 2, old_c, new_c);
...
{
	//return GetPixel(offscreen_DC,gYCenter + x,gYCenter - y);
...
{
	SetPixel(offscreen_DC, gXCenter + x, gYCenter - y,new_col);
}
